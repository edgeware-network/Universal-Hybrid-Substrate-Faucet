	...
	return (
		<div className="flex flex-col relative gap-[20px] min-h-[80vh] ">
			<div className="elements sm:flex hidden items-center p-4 rounded-md gap-[20px]">
				<button disabled={start === 0} onClick={() => { setStart(start - 10); setEnd(end - 10); }} className="flex items-center justify-center cursor-pointer">
				...
				<div className="grid auto-rows-auto h-[420px] overflow-y-auto w-[80vw] gap-3 grid-cols-3 ">
					{faucetBalances.map(async(chain, index) => (
						<div key={index} className={`flex flex-col items-center justify-center gap-3 ${chain.balance === null ? "bg-[#4d1526]" : "bg-black"} rounded-md text-center flex-wrap`}>
							<h3 className="text-[#eaeaea] text-sm">{chain.name}</h3>
							<h3 className="text-[#9b9b9b] text-sm">
								{formatBalances(chain.balance, chain.nativeCurrency.decimals)} {" "} {chain.nativeCurrency.symbol}
							</h3>
				...
				<button disabled={end === chains.length} onClick={() => { setStart(start + 10); setEnd(end + 10); }} className="flex items-center justify-center cursor-pointer">
