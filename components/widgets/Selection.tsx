...
export default function Selection({ switchChain, user, setUser, mode, options, value, onChange, onClose }: SelectionProps) {
  ...
  function selectOption(option: Chain) {
    if(mode){
      if (localStorage.getItem("SET_TYPE") === null) localStorage.setItem("SET_TYPE", JSON.stringify(option.type));
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col items-center justify-end space-y-5 p-1.5">
      <div className="flex items-center justify-start w-full">
        {mode && 
        <div className="text-[#9b9b9b] flex flex-grow flex-wrap items-center gap-[0.5em]">
          {value.map((chain) => (
            <div key={chain.url} className="flex items-center px-1.5 py-1 rounded-md bg-[#311C31] text-[#FC72FF] justify-between cursor-pointer">
              <span className="text-xs">{chain.name}</span>
              <span className="h-2 w-2 shrink-0 block" />
              <IoIosClose onClick={() => selectOption(chain)} className="h-4 w-4" />
            </div>
          ))}
        </div>}
      </div>
          ...
          <Fragment key={key}>
            ...
            <div className="grid text-sm gap-[0.5em] w-full grid-cols-2 font-medium items-center">
              {all_chains[key].map((chain) => (
                <div key={chain.url} className={`flex items-center ml-2 px-2 py-1 h-full rounded-md cursor-pointer ${mode ? `${isOptionSelected(chain) ? 'bg-[#311C31] text-[#FC72FF]' : 'hover:bg-[#191919] text-[#dadada]'}`: `${isOptionSelected(chain) ? 'bg-[rgba(0,102,255,0.1)] text-[#0066ff]' : 'hover:bg-[#191919] text-[#dadada]'}`}`} onClick={() => selectOption(chain)}>
                  <Image
      ...
      <button
        className={`w-1/4 text-[#fff] ${ mode ? 'bg-[rgba(252,114,255,0.7)] hover:bg-[rgba(252,114,255,0.9)]' : 'bg-[rgba(0,102,255,0.7)] hover:bg-[rgba(0,102,255,0.9)]' }  active:scale-95 p-2 rounded-[12px]`}
        type="submit"
      >
        Submit
      </button>
